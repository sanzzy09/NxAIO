...
type ViewMode = 'discover' | 'search' | 'detail' | 'read' | 'announcements' | 'genres';
...
export function ShinigamiExplorer() {
  ...
  const handleFetch = async (params: { mode: string; [key: string]: any }) => {
    ...
    try {
      ...
      if (!res.status) throw new Error(res.message || "Failed to sync with library.");
      setData(res.data);
  ...
  const getImageUrl = (manga: any) => {
    ...
    // Check various nested paths based on API response structure
      ...
      <div className="space-y-12 animate-fade-in-up">
        ...
        {list.length === 0 && !loading && (
          <div className="py-24 text-center space-y-4">
             <Info className="size-12 text-muted-foreground/20 mx-auto" />
             <p className="text-sm font-medium text-muted-foreground italic">No titles found in this category.</p>
          </div>
        )}
      </div>
    );
      ...
      <div className="space-y-10 animate-fade-in-up pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-4 space-y-8">
            <div className="relative aspect-[3/4] w-full rounded-[3rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-primary/5">
              <Image src={getImageUrl(data)} alt={data.title} fill className="object-cover" unoptimized />
            ...
            <div className="bg-secondary/20 p-8 rounded-[2.5rem] border border-primary/5 space-y-6">
               <h5 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/40">Series Analytics</h5>
               <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-primary/5">
                    ...
                    <span className="text-sm font-bold font-headline">{data.format || "Manhwa"}</span>
                  </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-primary/5 pb-4">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/40 flex items-center gap-2">
                  <ListOrdered className="size-3" /> Chapter Directory ({chapters.length})
                </h4>
                <Button variant="ghost" size="sm" className="text-[9px] font-bold uppercase tracking-widest gap-2">
                  ...
                  >
                    <div className="flex items-center gap-4">
                       ...
                       <div className="text-left">
                         ...
                         <span className="text-[9px] uppercase tracking-widest opacity-40 font-medium">{ch.release_date || "Synced recently"}</span>
                       </div>
                    </div>
      ...
      <div className="space-y-8 animate-fade-in-up pb-20">
        <div className="flex items-center justify-between sticky top-24 z-20 bg-background/80 backdrop-blur-xl p-4 rounded-[2rem] border border-primary/5 shadow-2xl">
          ...
          <div className="text-center">
            <h4 className="text-sm font-bold font-headline leading-tight line-clamp-1">{data.manga_title || "Chapter"}</h4>
            <span className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Chapter {data.chapter_number}</span>
          </div>

          <div className="flex gap-2">
            <Button variant="outline" size="icon" className="rounded-xl border-primary/5 shadow-sm">
        ...
        <div className="max-w-3xl mx-auto flex flex-col items-center bg-black/5 rounded-[3rem] overflow-hidden">
          ...
          {pages.length === 0 && !loading && (
            <div className="py-40 text-center space-y-4">
              <FileText className="size-12 text-muted-foreground/20 mx-auto" />
              <p className="text-sm font-medium text-muted-foreground italic">Syncing imagery with server... please wait.</p>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-center gap-4 pt-10">
           {data.prev_chapter_id && (
             <Button 
               variant="outline" 
               onClick={() => handleFetch({ mode: 'chapter', id: data.prev_chapter_id })}
               className="h-14 px-10 rounded-2xl font-bold gap-3 border-primary/10 hover:bg-primary hover:text-primary-foreground transition-all"
             >
           ...
           {data.next_chapter_id && (
             <Button 
               onClick={() => handleFetch({ mode: 'chapter', id: data.next_chapter_id })}
               className="h-14 px-10 rounded-2xl font-bold gap-3 shadow-xl shadow-primary/10 transition-all"
             >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Premium manga & manhwa directory with cinematic reader.</CardDescription>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Tabs 
              value={view === 'discover' ? 'discover' : 'library'} 
              onValueChange={(v) => {
                if (v === 'discover') setView('discover');
              }}
      ...
      <CardContent className="p-8 sm:p-10 pt-0 space-y-8 min-h-[500px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 space-y-4">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-40">Synchronizing neural data...</p>
          </div>
        ) : error ? (
