...
const SUPPORTED_PLATFORMS = [
  "TikTok", "Douyin", "Kuaishou", "Xiaohongshu", "Bilibili", "Ixigua", "Toutiao", 
  "Weibo", "Pipix", "Izuiyou", "PearVideo", "Xinpianchang", "Haokan", "Huya", "AcFun", 
  "Instagram", "Facebook", "Bluesky", "CapCut", "Dailymotion", "LinkedIn", "Pinterest", 
  "Reddit", "Snapchat", "SoundCloud", "Spotify", "Terabox", "Threads", "Tumblr", 
  "Twitter (X)", "YouTube"
];
    ...
    <Card className="border-none shadow-sm bg-card/50 backdrop-blur-md overflow-hidden rounded-[2.5rem]">
      ...
      <CardContent className="p-8 sm:p-10 pt-0 space-y-8">
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-[2rem] bg-secondary/20 border border-primary/5 space-y-4 animate-fade-in-up">
            <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50 flex items-center gap-2">
              <Globe className="w-3 h-3" /> Supported Platforms
            </h4>
            <div className="flex flex-wrap gap-1.5">
              {SUPPORTED_PLATFORMS.map((platform) => (
                <span 
                  key={platform} 
                  className="text-[10px] px-2.5 py-1 rounded-full bg-background/50 text-muted-foreground border border-primary/5 hover:border-primary/20 transition-colors"
                >
                  {platform}
                </span>
              ))}
              <span className="text-[10px] px-2.5 py-1 text-muted-foreground/40 italic">
                & many more...
              </span>
            </div>
          </div>
